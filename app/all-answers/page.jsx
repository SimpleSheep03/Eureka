...
const AllAnswers = () => {
    ...
    const fetchSolutions = async () => {
      try {
        const res = await fetch('/api/fetchSolutions', {
          ...
          body: JSON.stringify({ pageNo: 1 , size : 10 , handle : session?.username}), // Start at page 1
        });
  ...
  return (
    <div className="flex justify-center items-center md:mt-8 md:p-10">
      ...
      <Toaster />
      <h2 className="text-2xl font-semibold text-center mb-5">Answers</h2>
      {solutions.length > 0 ? (
        <ul className="space-y-4">
          {solutions.map((solution) => (
            <AnswerCard key={solution._id} answer={solution} reactions={reactions}/>
          ))}
        </ul>
      ) : (
      ...
      <div className="mt-4 flex items-center justify-center">
        <button
          ...
          disabled={pageNo === 1}
          className={`flex items-center text-white hover:underline mr-7 ${pageNo === 1 ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          <FaLongArrowAltLeft />
        ...
        <button
          onClick={() => handlePageChange(pageNo + 1)}
          disabled={lastPage}
          className={`flex items-center text-white hover:underline ${lastPage ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
